  ...
  function renderUI(id, target) {
    ...
    target.prepend(
      '<div class="plupload_wrapper plupload_scroll">' +
        '<div id="' +
        id +
        '_container" class="plupload_container">' +
        '<div class="plupload">' +
        ...
        '<a href="#" class="plupload_button plupload_add" id="' +
        id +
        '_browse">' +
        _("Add Files") +
        "</a>" +
    ...
    if (settings) {
      this.each(function () {
        ...
        function updateTotalProgress() {
          $("span.plupload_total_status", target).html(
            uploader.total.percent + "%"
          );
          $("div.plupload_progress_bar", target).css(
            "width",
            uploader.total.percent + "%"
          );
          $("span.plupload_upload_status", target).html(
            _("Uploaded %d/%d files").replace(
              /%d\/%d/,
        ...
        function updateList() {
          ...
          $("span.plupload_total_file_size", target).html(
            plupload.formatSize(uploader.total.size)
          );

          if (uploader.total.queued === 0) {
            $("span.plupload_add_text", target).html(_("Add Files"));
          } else {
            $("span.plupload_add_text", target).html(
              _("%d files queued").replace(/%d/, uploader.total.queued)
            );
          }

          $("a.plupload_start", target).toggleClass(
            "plupload_disabled",
          ...
          if (
            !uploader.files.length &&
            uploader.features.dragdrop &&
            uploader.settings.dragdrop
          ) {
            $("#" + id + "_filelist").append(
              '<li class="plupload_droptext">' + _("Drag files here.") + "</li>"
            );
          }
        }
        ...
        uploader.bind("Init", function (up, res) {
          ...
          if (!settings.unique_names && settings.rename) {
            target.on(
              "click",
              "#" + id + "_filelist div.plupload_file_name span",
              function (e) {
                ...
                targetSpan
                  .hide()
                  .after('<input type="text" autocomplete="off"  />');
                targetSpan
                  .next()
                  .val(name)
                  .focus()
        ...
        uploader.bind("StateChanged", function () {
          if (uploader.state === plupload.STARTED) {
            ...
            if (
              settings.multiple_queues &&
              uploader.total.uploaded + uploader.total.failed ==
                uploader.files.length
            ) {
              $(".plupload_buttons,.plupload_upload_status", target).css(
                "display",
                "inline"
              );
        ...
        uploader.bind("UploadProgress", function (up, file) {
          ...
          $("#" + file.id + " div.plupload_file_status", target).html(
            file.percent + "%"
          );
