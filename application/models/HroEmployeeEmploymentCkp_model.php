<?php
	class HroEmployeeEmploymentCkp_model extends CI_Model {
		...
		public function getCoreWorkAccident(){
			...
			$this->db->where('core_work_accident.data_state',0);
			$result = $this->db->get()->result_array();
			return $result;			
		}
		...
		public function getHROEmployeeData_Employment($region_id, $branch_id, $location_id, $division_id, $department_id, $section_id){
		...
		public function getHROEmployeeSuspend($employee_id){
			$this->db->select('hro_employee_suspend.employee_suspend_id, hro_employee_suspend.employee_id, hro_employee_data.employee_code, hro_employee_data.employee_name, hro_employee_suspend.employee_suspend_date, hro_employee_suspend.suspend_id, core_suspend.suspend_name, hro_employee_suspend.employee_suspend_description, hro_employee_suspend.employee_suspend_days, hro_employee_suspend.employee_suspend_status, hro_employee_suspend.employee_suspend_status_date, hro_employee_suspend.employee_suspend_salary_percentage, hro_employee_suspend.employee_suspend_remark');
	...
	/*	public function getHROEmployeeWorkAccident($employee_id){
			$this->db->select('hro_employee_work_accident.employee_work_accident_id, hro_employee_work_accident.employee_id, hro_employee_work_accident.work_accident_id, core_work_accident.work_accident_name, hro_employee_work_accident.employee_work_accident_description, hro_employee_work_accident.employee_work_accident_date, hro_employee_work_accident.employee_work_accident_duration, hro_employee_work_accident.employee_work_accident_start_date, hro_employee_work_accident.employee_work_accident_end_date');
			$this->db->from('hro_employee_work_accident');
			$this->db->join('core_work_accident', 'hro_employee_work_accident.work_accident_id = core_work_accident.work_accident_id');
			$this->db->where('hro_employee_work_accident.data_state', 0);
			$this->db->where('hro_employee_work_accident.employee_id', $employee_id);
			$this->db->order_by('hro_employee_work_accident.employee_work_accident_id', 'DESC');
			//$result = $this->db->get()->result_array();
			//return $result;
		}*/
		...
		public function updateHROEmployeeAttendanceData_Status($data){
			$this->db->where('hro_employee_attendance_data.employee_attendance_date', $data['employee_attendance_date']);
		...
		public function insertHROEmployeeWorkAccident($data){
			return $this->db->insert('hro_employee_work_accident',$data);
		}

		public function getEmployeeWorkAccidentID($created_id){
			$this->db->select('hro_employee_work_accident.employee_work_accident_id');
			$this->db->from('hro_employee_work_accident');
			$this->db->where('hro_employee_work_accident.created_id', $created_id);
			$this->db->where('hro_employee_work_accident.data_state',0);
			$this->db->order_by('hro_employee_work_accident.employee_work_accident_id', DESC);
			$this->db->limit(1);
			$result=$this->db->get()->row_array();
			return $result['employee_work_accident_id'];
		}

		public function insertHROEmployeeWorkAccident_Detail($data){
			return $this->db->insert('hro_employee_work_accident_detail', $data);
		}

		public function deleteHROEmployeeWorkAccident($data){
			$this->db->where("hro_employee_work_accident.employee_work_accident_id", $data['employee_work_accident_id']);
			$query = $this->db->update('hro_employee_work_accident', $data);
			if($query){
				return true;
			}else{
				return false;
