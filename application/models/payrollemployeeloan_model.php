<?php
	class payrollemployeeloan_model extends CI_Model {
		var $table = "hro_employee_loan";
		
		public function payrollemployeeloan_model(){
			parent::__construct();
		...
		public function deletePayrollEmployeeLoan_Data($employee_loan_id){
			$this->db->where("payroll_employee_loan.employee_loan_id", $employee_loan_id);
			$query = $this->db->update('payroll_employee_loan', array("data_state"=>1));
			if($query){
				return true;
			}else{
				return false;
