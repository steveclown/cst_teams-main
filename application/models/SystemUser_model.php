<?php
	class SystemUser_model extends CI_Model {
		...
		public function getSystemUser_DetailEdit($user_id){
			$this->db->select('system_user.user_id,system_user.avatar, system_user.employee_id, system_user.region_id, system_user.branch_id, system_user.location_id, system_user.division_id, system_user.department_id, system_user.section_id, system_user.username, system_user.password, system_user.user_group_id, system_user.employee_employment_working_status, system_user.user_status');
			$this->db->from('system_user');
			$this->db->where('system_user.user_id', $user_id);
