<?php
	class payrollemployeemonthlyilufa_model extends CI_Model {
		var $table = "hro_employee_allowance";
		
		public function payrollemployeemonthlyilufa_model(){
			parent::__construct();
		...
		public function getHROEmployeeData($employee_id){
			$this->db->select('hro_employee_data.employee_id, hro_employee_data.employee_name, hro_employee_data.division_id, hro_employee_data.department_id, hro_employee_data.section_id, hro_employee_data.employee_hire_date, hro_employee_data.employee_employment_working_status');
		...
		public function getHROEmployeeData_Monthly($region_id, $data_branch, $branch_status, $branch_id, $payroll_employee_level, $division_id, $department_id, $section_id){
		...
		public function getPayrollEmployeeBPJS($employee_id){
			$this->db->select('payroll_employee_bpjs.employee_bpjs_id, payroll_employee_bpjs.bpjs_total_amount, payroll_employee_bpjs.bpjs_kesehatan_amount, payroll_employee_bpjs.bpjs_tenagakerja_amount, payroll_employee_bpjs.bpjs_status');
		...
		public function getPayrollLeaveRequest($employee_id, $start_date, $end_date, $annual_leave_id){
		...
		public function getHROEmployeePermit($employee_id, $start_date, $end_date, $permit_id){
			$this->db->select('hro_employee_permit.employee_permit_id, hro_employee_permit.permit_id, hro_employee_permit.employee_id, hro_employee_permit.permit_type, hro_employee_permit.deduction_type, hro_employee_permit.employee_permit_description, hro_employee_permit_detail.employee_permit_detail_id, hro_employee_permit_detail.employee_permit_detail_date, core_permit.deduction_id, core_deduction.deduction_type');
		...
		public function getCoreDeduction_Detail($deduction_id){
			$this->db->select('core_deduction.deduction_id, core_deduction.deduction_type, core_deduction.deduction_amount, core_deduction.deduction_premi_attendance_ratio, core_deduction.deduction_premi_attendance_status, core_deduction.deduction_basic_salary_ratio, core_deduction.deduction_late_start_duration, core_deduction.deduction_late_end_duration, core_deduction.deduction_type');
		...
		public function getPayrollEmployeeDeduction_Detail($employee_id, $employee_deduction_period){
			$this->db->select('payroll_employee_deduction.employee_deduction_id, payroll_employee_deduction.employee_id, payroll_employee_deduction.deduction_id, payroll_employee_deduction.employee_deduction_period, payroll_employee_deduction.employee_deduction_amount, core_deduction.deduction_type, core_deduction.deduction_amount, core_deduction.deduction_premi_attendance_ratio, core_deduction.deduction_premi_attendance_status, core_deduction.deduction_basic_salary_ratio,  core_deduction.deduction_late_start_duration, core_deduction.deduction_late_end_duration');
			$this->db->from('payroll_employee_deduction');
			$this->db->join('core_deduction', 'payroll_employee_deduction.deduction_id = core_deduction.deduction_id');
			$this->db->where('payroll_employee_deduction.data_state', 0);
			$this->db->where('payroll_employee_deduction.employee_id', $employee_id);
			$this->db->where('payroll_employee_deduction.employee_deduction_period', $employee_deduction_period);
			$this->db->where('core_deduction.deduction_type', 0);
			$result = $this->db->get()->result_array();
			return $result;
		}
		...
		public function saveNewPayrollEmployeeMonthlySaving($data){
			return $this->db->insert('payroll_employee_monthly_saving',$data);
		}

		public function saveNewPayrollEmployeeMonthlyLoan($data){
			return $this->db->insert('payroll_employee_monthly_loan',$data);
		}

		public function saveNewPayrollEmployeeMonthlyBonus($data){
			return $this->db->insert('payroll_employee_monthly_bonus',$data);
		}

		public function saveNewPayrollEmployeeMonthlyLostItem($data){
			return $this->db->insert('payroll_employee_monthly_lost_item',$data);
		}

		public function saveNewPayrollEmployeeMonthlyIncentive($data){
			return $this->db->insert('payroll_employee_monthly_incentive',$data);
		}

		public function saveNewPayrollEmployeeMonthlyCommission($data){
			return $this->db->insert('payroll_employee_monthly_commission',$data);
		}

		public function saveNewPayrollEmployeeMonthlyDeductionPremi($data){
			return $this->db->insert('payroll_employee_monthly_deduction_premi',$data);
		}

		public function getPayrollEmployeeMonthly_Data($employee_id){
			$this->db->select('payroll_employee_monthly.employee_monthly_id, payroll_employee_monthly.employee_id, payroll_employee_monthly.employee_monthly_period, payroll_employee_monthly.employee_monthly_working_days, payroll_employee_monthly.employee_monthly_basic_salary, payroll_employee_monthly.employee_monthly_allowance_total_amount, payroll_employee_monthly.employee_monthly_deduction_total_amount, payroll_employee_monthly.employee_monthly_overtime_total_amount, payroll_employee_monthly.employee_monthly_home_early_total_amount, payroll_employee_monthly.employee_monthly_bpjs_amount, payroll_employee_monthly.employee_monthly_allowance_other, payroll_employee_monthly.employee_monthly_deduction_other, payroll_employee_monthly.employee_monthly_salary_total_amount');
			$this->db->from('payroll_employee_monthly');
		...
		public function getEmployeeWorkingStatus($employee_id){
			$this->db->select('hro_employee_data.employee_employment_working_status');
			$this->db->from('hro_employee_data');
			$this->db->where('hro_employee_data.employee_id', $employee_id);
			$result=$this->db->get()->row_array();
			return $result['employee_employment_working_status'];
		}

		public function getEmployeeEmploymentStatus($employee_id){
			$this->db->select('hro_employee_data.employee_employment_status');
			$this->db->from('hro_employee_data');
			$this->db->where('hro_employee_data.employee_id', $employee_id);
			$result=$this->db->get()->row_array();
			return $result['employee_employment_status'];
		}

		public function getCoreLengthService(){
			$this->db->select('core_length_service.length_service_range1, core_length_service.length_service_range2, core_length_service.length_service_amount_multiply, core_length_service.length_service_min_saving');
			$this->db->from('core_length_service');
			$this->db->where('core_length_service.data_state', 0);
			$this->db->order_by('core_length_service.length_service_id', 'DESC');
			$this->db->limit(1);
			$result = $this->db->get()->row_array();
			return $result;
		}

		public function getPayrollEmployeeLengthService($employee_id, $year_period){
			$this->db->select('payroll_employee_length_service.employee_length_service_id, payroll_employee_length_service.employee_id, payroll_employee_length_service.length_service_id,  core_length_service.length_service_range1, core_length_service.length_service_range2, core_length_service.length_service_amount_multiply, core_length_service.length_service_min_saving');
		...
		public function getPayrollEmployeeBonus($employee_id, $employee_bonus_period){
			$this->db->select('payroll_employee_bonus.employee_id, payroll_employee_bonus.bonus_id, payroll_employee_bonus.employee_bonus_amount, payroll_employee_bonus.employee_bonus_description');
			...
			$this->db->where('payroll_employee_bonus.employee_bonus_period', $employee_bonus_period);
			$result = $this->db->get()->result_array();
			return $result;
		}

		public function getPayrollEmployeeLostItem($employee_id, $employee_lost_item_period){
			$this->db->select('payroll_employee_lost_item.employee_id, payroll_employee_lost_item.lost_item_id, payroll_employee_lost_item.employee_lost_item_period, payroll_employee_lost_item.employee_lost_item_amount, payroll_employee_lost_item.employee_lost_item_description');
			...
			$this->db->where('payroll_employee_lost_item.employee_lost_item_period', $employee_lost_item_period);
			$result = $this->db->get()->result_array();
			return $result;
		}

		public function getPayrollEmployeeCommission($employee_id, $employee_commission_period){
			$this->db->select('payroll_employee_commission.employee_id, payroll_employee_commission.employee_commission_omzet_mmc, payroll_employee_commission.employee_commission_quantity_mmc, payroll_employee_commission.employee_commission_omzet_acc, payroll_employee_commission.employee_commission_total_omzet, payroll_employee_commission.employee_commission_amount_mmc, payroll_employee_commission.employee_commission_amount_acc, payroll_employee_commission.employee_commission_total_amount');
		...
		public function getPayrollEmployeeIncentive($employee_id, $employee_incentive_period){
			$this->db->select('payroll_employee_incentive.employee_id, payroll_employee_incentive.incentive_id, core_incentive.incentive_name, payroll_employee_incentive.employee_incentive_period, payroll_employee_incentive.employee_incentive_amount');
			$this->db->from('payroll_employee_incentive');
			$this->db->join('core_incentive', 'payroll_employee_incentive.incentive_id = core_incentive.incentive_id');
			$this->db->where('payroll_employee_incentive.data_state', 0);
			$this->db->where('payroll_employee_incentive.employee_id', $employee_id);
			$this->db->where('payroll_employee_incentive.employee_incentive_period', $employee_incentive_period);
			$result = $this->db->get()->result_array();
			/*print_r($this->db->last_query());
			exit;*/
			return $result;
		...
		public function getPayrollEmployeeDeductionPremi($employee_id, $employee_deduction_premi_period){
			$this->db->select('payroll_employee_deduction_premi.employee_id, payroll_employee_deduction_premi.premi_attendance_id, core_premi_attendance.premi_attendance_name, payroll_employee_deduction_premi.employee_deduction_premi_period, payroll_employee_deduction_premi.employee_deduction_premi_amount, payroll_employee_deduction_premi.employee_deduction_premi_description');
			...
			$this->db->where('payroll_employee_deduction_premi.employee_deduction_premi_period', $employee_deduction_premi_period);
			$result = $this->db->get()->result_array();
			/*print_r($this->db->last_query());
			exit;*/
			return $result;
		...
		public function getPayrollEmployeeLoan($employee_id, $monthly_period){
			$this->db->select('payroll_employee_loan.employee_id, payroll_employee_loan.loan_type_id, core_loan_type.loan_type_name, payroll_employee_loan.employee_loan_status, payroll_employee_loan_item.employee_loan_item_amount, payroll_employee_loan_item.employee_loan_item_period');
			$this->db->from('payroll_employee_loan');
			$this->db->join('payroll_employee_loan_item', 'payroll_employee_loan.employee_loan_id = payroll_employee_loan_item.employee_loan_id');
		...
		public function getCoreAnnualLeave(){
			$this->db->select('core_annual_leave.annual_leave_id, core_annual_leave.annual_leave_name, core_annual_leave.deduction_id, core_deduction.deduction_type');
			$this->db->from('core_annual_leave');
			$this->db->join('core_deduction', 'core_annual_leave.deduction_id = core_deduction.deduction_id');
		...
		public function getPayrollEmployeeMonthly($region_id, $data_branch, $branch_status, $branch_id, $payroll_employee_level, $employee_monthly_period){
			$this->db->select('payroll_employee_monthly.employee_monthly_id, payroll_employee_monthly.employee_monthly_period, payroll_employee_monthly.employee_monthly_start_date, payroll_employee_monthly.employee_monthly_end_date, payroll_employee_monthly.employee_id, hro_employee_data.employee_name, hro_employee_data.branch_id, core_branch.branch_name, hro_employee_data.section_id, core_section.section_name, hro_employee_data.job_title_id, core_job_title.job_title_name, payroll_employee_monthly.bank_id, core_bank.bank_name, payroll_employee_monthly.employee_monthly_bank_acct_no, payroll_employee_monthly.employee_monthly_bank_acct_name, payroll_employee_monthly.employee_monthly_basic_daily_salary, payroll_employee_monthly.employee_monthly_basic_salary, payroll_employee_monthly.employee_monthly_basic_salary_amount, payroll_employee_monthly.employee_monthly_basic_overtime, payroll_employee_monthly.employee_monthly_working_days, payroll_employee_monthly.employee_monthly_allowance_total_amount, payroll_employee_monthly.employee_monthly_deduction_total_amount, payroll_employee_monthly.employee_monthly_overtime_total_amount, payroll_employee_monthly.employee_monthly_bpjs_amount, payroll_employee_monthly.employee_monthly_length_service_amount, payroll_employee_monthly.employee_monthly_length_saving_amount, payroll_employee_monthly.employee_monthly_premi_attendance_amount, payroll_employee_monthly.employee_monthly_incentive_total_amount, payroll_employee_monthly.employee_monthly_loan_total_amount, payroll_employee_monthly.employee_monthly_bonus_total_amount, payroll_employee_monthly.employee_monthly_lost_item_total_amount, payroll_employee_monthly.employee_monthly_commission_total_amount, payroll_employee_monthly.employee_monthly_deduction_premi_amount, payroll_employee_monthly.employee_monthly_salary_total_amount');
			$this->db->from('payroll_employee_monthly');
			...
			$this->db->join('core_bank', 'payroll_employee_monthly.bank_id = core_bank.bank_id');
			...
			if($branch_status != 9 ){
				if ($branch_id == ''){
					$this->db->where_in('payroll_employee_monthly.branch_id', $data_branch);
				} else {
			...
			$this->db->order_by('hro_employee_data.branch_id', 'ASC');
			$this->db->order_by('hro_employee_data.section_id', 'ASC');
			$result = $this->db->get()->result_array();
			return $result;
		}

		public function getPayrollEmployeeMonthlyPeriod($region_id, $branch_id, $location_id, $payroll_employee_level){
			$this->db->distinct();
			...
			$this->db->where('payroll_employee_monthly.payroll_employee_level', $payroll_employee_level);
			$result = $this->db->get()->result_array();
			return $result;
		}

		public function getPayrollEmployeeMonthlyAllowance_Detail($employee_id, $employee_monthly_period){
			$this->db->select('payroll_employee_monthly_allowance.employee_monthly_allowance_id, payroll_employee_monthly_allowance.allowance_id, core_allowance.allowance_name, payroll_employee_monthly_allowance.employee_allowance_amount, payroll_employee_monthly_allowance.employee_monthly_working_days, payroll_employee_monthly_allowance.employee_monthly_allowance_days, payroll_employee_monthly_allowance.employee_allowance_subtotal');
			$this->db->from('payroll_employee_monthly_allowance');
			$this->db->join('core_allowance', 'payroll_employee_monthly_allowance.allowance_id = core_allowance.allowance_id');
			$this->db->where('payroll_employee_monthly_allowance.data_state',0);
			$this->db->where('payroll_employee_monthly_allowance.employee_id', $employee_id);
			$this->db->where('payroll_employee_monthly_allowance.employee_monthly_period', $employee_monthly_period);
			$result = $this->db->get()->result_array();
			return $result;
		}

		public function getPayrollEmployeeMonthlyDeduction_Detail($employee_id, $employee_monthly_period){
			$this->db->select('payroll_employee_monthly_deduction.employee_monthly_deduction_id, payroll_employee_monthly_deduction.employee_deduction_id, payroll_employee_monthly_deduction.deduction_id, core_deduction.deduction_name, core_deduction.deduction_type, payroll_employee_monthly_deduction.employee_deduction_amount, payroll_employee_monthly_deduction.employee_monthly_working_days, payroll_employee_monthly_deduction.employee_monthly_deduction_days, payroll_employee_monthly_deduction.employee_deduction_subtotal');
			$this->db->from('payroll_employee_monthly_deduction');
			$this->db->join('core_deduction', 'payroll_employee_monthly_deduction.deduction_id = core_deduction.deduction_id');
			$this->db->where('payroll_employee_monthly_deduction.data_state',0);
			$this->db->where('payroll_employee_monthly_deduction.employee_id', $employee_id);
			$this->db->where('payroll_employee_monthly_deduction.employee_monthly_period', $employee_monthly_period);
			$result = $this->db->get()->result_array();
			return $result;
		}
