<?php
	class assignmentbusinesstripspa_model extends CI_Model {
		var $table = "transaction_business_trip";
		
		public function assignmentbusinesstripspa_model(){
			parent::__construct();
		...
		public function getOvertimeRateAmount($overtime_rate_id, $division_id, $department_id, $section_id, $job_title_id){
			$this->db->select('assignment_overtime_rate_title.overtime_rate_amount');
			$this->db->from('assignment_overtime_rate_title');
			$this->db->where('assignment_overtime_rate_title.overtime_rate_id', $overtime_rate_id);
			$this->db->where('assignment_overtime_rate_title.division_id', $division_id);
			$this->db->where('assignment_overtime_rate_title.department_id', $department_id);
			$this->db->where('assignment_overtime_rate_title.section_id', $section_id);
			$this->db->where('assignment_overtime_rate_title.job_title_id', $job_title_id);
			$result=$this->db->get()->row_array();
		...
		public function insertAssignmentBusinessTrip($data){
			return $this->db->insert('assignment_business_trip_spa',$data);
		}

		public function getBusinessTripID($created_on, $created_id){
			$this->db->select('assignment_business_trip_spa.business_trip_id');
			$this->db->from('assignment_business_trip_spa');
			$this->db->where('assignment_business_trip_spa.created_id', $created_id);
			$this->db->order_by('assignment_business_trip_spa.created_on', 'DESC');
			$this->db->limit(1);
			$result = $this->db->get()->row_array();
		...
		public function getAssignmentBusinessTrip_Detail($business_trip_id){
			$this->db->select('assignment_business_trip_spa.business_trip_id, assignment_business_trip_spa.division_id, core_division.division_name, assignment_business_trip_spa.department_id, core_department.department_name, assignment_business_trip_spa.section_id, core_section.section_name, assignment_business_trip_spa.job_title_id, core_job_title.job_title_name, assignment_business_trip_spa.employee_id, hro_employee_data.employee_name, assignment_business_trip_spa.business_trip_date, assignment_business_trip_spa.business_trip_start_date, assignment_business_trip_spa.business_trip_end_date, assignment_business_trip_spa.business_trip_purpose, assignment_business_trip_spa.business_trip_status, assignment_business_trip_spa.approved, assignment_business_trip_spa.returned, assignment_business_trip_spa.overtime_rate_id, assignment_overtime_rate.overtime_rate_description');
			$this->db->from('assignment_business_trip_spa');
			$this->db->join('core_division', 'assignment_business_trip_spa.division_id = core_division.division_id');
			$this->db->join('core_department', 'assignment_business_trip_spa.department_id  = core_department.department_id');
			$this->db->join('core_section', 'assignment_business_trip_spa.section_id = core_section.section_id');
			$this->db->join('core_job_title', 'assignment_business_trip_spa.job_title_id = core_job_title.job_title_id');
			$this->db->join('hro_employee_data', 'assignment_business_trip_spa.employee_id = hro_employee_data.employee_id');
			$this->db->join('assignment_overtime_rate', 'assignment_business_trip_spa.overtime_rate_id = assignment_overtime_rate.overtime_rate_id');
			$this->db->where('assignment_business_trip_spa.data_state', 0);
			$this->db->where('assignment_business_trip_spa.business_trip_id', $business_trip_id);
			$result = $this->db->get()->row_array();
			return $result;
		}

		public function getAssignmentBusinessTrip_Data($employee_id){
			$this->db->select('assignment_business_trip_spa.business_trip_id, assignment_business_trip_spa.employee_id, assignment_business_trip_spa.business_trip_date, assignment_business_trip_spa.business_trip_destination, assignment_business_trip_spa.overtime_rate_id, assignment_overtime_rate.overtime_rate_description, assignment_business_trip_spa.business_trip_amount');
			$this->db->from('assignment_business_trip_spa');
			$this->db->join('assignment_overtime_rate', 'assignment_business_trip_spa.overtime_rate_id = assignment_overtime_rate.overtime_rate_id');
			$this->db->where('assignment_business_trip_spa.data_state',0);
			$this->db->where('assignment_business_trip_spa.employee_id', $employee_id);
			$this->db->order_by('assignment_business_trip_spa.business_trip_id', DESC);
			$result = $this->db->get()->result_array();
			return $result;
		}
		...
		public function approvedAssignmentBusinessTrip($data){
			...
			if($this->db->update('assignment_business_trip_spa', $data)){
				return true;
			} else {
				return false;
			}
		...
		public function deleteAssignmentBusinessTrip_Data($business_trip_id){
			$this->db->where("assignment_business_trip_spa.business_trip_id", $business_trip_id);
			$query = $this->db->update('assignment_business_trip_spa', array("data_state"=>1));
			if($query){
				return true;
			}else{
				return false;
