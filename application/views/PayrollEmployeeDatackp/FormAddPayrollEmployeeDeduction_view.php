<script>
	...
	function reset_add_deduction(){
		document.location = base_url+"payrollemployeedatackp/reset_add_deduction/<?php echo $hroemployeedata['employee_id']; ?>";
	}

	function function_elements_add_deduction(name, value){
		$.ajax({
				type: "POST",
				url : "<?php echo site_url('payrollemployeedatackp/function_elements_add_deduction');?>",
...
<?php 
	echo form_open('payrollemployeedatackp/processAddPayrollEmployeeDeduction',array('id' => 'myform', 'class' => 'horizontal-form')); 
						...
						foreach ($payrollemployeededuction as $key=>$val){
							echo"
								<tr>
									...
									<td>
										<a href='".$this->config->item('base_url').'payrollemployeedatackp/deletePayrollEmployeeDeduction/'.$val['employee_id']."/".$val['employee_deduction_id']."' onClick='javascript:return confirm(\"Are you sure you want to delete this entry ?\")' class='btn default btn-xs red'>
