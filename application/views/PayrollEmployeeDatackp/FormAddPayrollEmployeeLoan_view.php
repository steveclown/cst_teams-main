<script>
	function reset_add_loan(){
		document.location = base_url+"PayrollEmployeeDatackp/reset_add_loan/<?php echo $hroemployeedata['employee_id']; ?>";
	}

	function function_elements_add_loan(name, value){
		$.ajax({
				type: "POST",
				url : "<?php echo site_url('PayrollEmployeeDatackp/function_elements_add_loan');?>",
...
<?php 
	echo form_open('PayrollEmployeeDatackp/processAddPayrollEmployeeLoan',array('id' => 'myform', 'class' => 'horizontal-form')); 
	...
	if(empty($data['employee_loan_date'])){
		$data['employee_loan_date']				= date('Y-m-d');
	}

	if(empty($data['employee_loan_description'])){
		$data['employee_loan_description']		= '';
	}

	if(empty($data['loan_month_start'])){
		$data['loan_month_start']				= '';
	}

	if(empty($data['loan_year_start'])){
		$data['loan_year_start']				= '';
	}

	if(empty($data['employee_loan_amount_total'])){
		$data['employee_loan_amount_total']		= '';
	}

	if(empty($data['employee_loan_amount'])){
		$data['employee_loan_amount']			= '';
	}
?>
<div class = "row">
	<div class = "col-md-6">
						...
						foreach ($payrollemployeeloan as $key=>$val){
							echo"
								<tr>
									...
									<td>
										<a href='".$this->config->item('base_url').'PayrollEmployeeDatackp/deletePayrollEmployeeLoan/'.$val['employee_id']."/".$val['employee_loan_id']."' onClick='javascript:return confirm(\"Are you sure you want to delete this entry ?\")' class='btn default btn-xs red'>
