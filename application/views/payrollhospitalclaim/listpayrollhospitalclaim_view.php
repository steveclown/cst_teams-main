<script>
    ...
    function reset_filter(){
		document.location = base_url+"payrollhospitalclaim/reset_search";
	}
</script>
...
?>
			...
			<h1 class="page-title">
				Hospital Claim List <small>Manage Hospital Claim</small>
			</h1>
			<!-- END PAGE TITLE & BREADCRUMB-->

<?php echo form_open('payrollhospitalclaim/filter',array('id' => 'myform', 'class' => '')); ?>
<div class="row">
	<div class="col-md-12">
				...
				<div class="portlet-body">
					<table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1">
						...
						<tbody>
							<?php
								$no=1;
								foreach ($hroemployeedata_hospitalclaim as $key=>$val){
									...
									echo"
										<tr>			
											<td>".$no."</td>						
											<td>".$this->payrollhospitalclaim_model->getEmployeeName($val['employee_id'])."</td>
											<td>".$this->payrollhospitalclaim_model->getDivisionName($val['division_id'])."</td>
											<td>".$this->payrollhospitalclaim_model->getDepartmentName($val['department_id'])."</td>
											<td>".$this->payrollhospitalclaim_model->getSectionName($val['section_id'])."</td>
											<td>
												<a href='".$this->config->item('base_url').'payrollhospitalclaim/addPayrollHospitalClaim/'.$val['employee_id']."' class='btn default btn-xs green-jungle'>
													<i class='fa fa-plus'></i> Add
												</a>
												<a href='".$this->config->item('base_url').'payrollhospitalclaim/deletePayrollHospitalClaim/'.$val['employee_id']."' onClick='javascript:return confirm(\"Are you sure you want to delete this entry ?\")' class='btn default btn-xs red'>
