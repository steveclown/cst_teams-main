<script>
    ...
    function reset_filter(){
		document.location = base_url+"hroemployeeperformance/reset_search";
	}
</script>
...
<?php echo form_open('hroemployeeperformance/filter',array('id' => 'myform', 'class' => '')); ?>
<div class="row">
	<div class="col-md-12">
							...
							<?php
								...
								foreach ($hroemployeedata_asset as $key=>$val){
									...
									echo"
										<tr>			
											<td>".$no."</td>						
											<td>".$this->hroemployeeperformance_model->getEmployeeName($val['employee_id'])."</td>
											<td>".$this->hroemployeeperformance_model->getDivisionName($val['division_id'])."</td>
											<td>".$this->hroemployeeperformance_model->getDepartmentName($val['department_id'])."</td>
											<td>".$this->hroemployeeperformance_model->getSectionName($val['section_id'])."</td>
											<td>
												<a href='".$this->config->item('base_url').'hroemployeeperformance/detailHROEmployeePerformance/'.$val['employee_id']."' class='btn default btn-xs yellow-lemon'>
