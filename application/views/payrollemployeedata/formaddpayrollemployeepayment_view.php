<script>
	...
	function reset_add_payment(){
	 	/*alert('asd');*/
		document.location = base_url+"payrollemployeedata/reset_add_payment/<?php echo $hroemployeedata['employee_id']; ?>";
	}

	function function_elements_add_payment(name, value){
		$.ajax({
				type: "POST",
				url : "<?php echo site_url('payrollemployeedata/function_elements_add_payment');?>",
...
<?php 
	echo form_open('payrollemployeedata/processAddPayrollEmployeePayment',array('id' => 'myform', 'class' => 'horizontal-form')); 
						...
						foreach ($payrollemployeepayment as $key=>$val){
							echo"
								<tr>
									...
									<td>
										<a href='".$this->config->item('base_url').'payrollemployeedata/deletePayrollEmployeePayment/'.$val['employee_id']."/".$val['employee_payment_id']."' onClick='javascript:return confirm(\"Are you sure you want to delete this entry ?\")' class='btn default btn-xs red'>
