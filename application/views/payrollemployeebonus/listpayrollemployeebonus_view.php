...
<script>
    ...
    function reset_filter(){
		document.location = base_url+"payrollemployeebonus/reset_search";
	}
	...
	$(document).ready(function(){
        $("#division_id").change(function(){
            ...
            $.ajax({
               type : "POST",
               url  : "<?php echo base_url(); ?>payrollemployeebonus/getCoreDepartment",
    ...
    $(document).ready(function(){
        $("#department_id").change(function(){
            ...
            $.ajax({
               type : "POST",
               url  : "<?php echo base_url(); ?>payrollemployeebonus/getCoreSection",
...
<?php 
	$data = $this->session->userdata('filter-payrollemployeebonus',$data);
	echo form_open('payrollemployeebonus/filter',array('id' => 'myform', 'class' => '')); 
?>
<div class="row">
	<div class="col-md-12">
							...
							<div class="form-group form-md-line-input">
								<?php
									if (!empty($data['division_id'])){
										$coredepartment = create_double($this->payrollemployeebonus_model->getCoreDepartment($data['division_id']), 'department_id', 'department_name');
							...
							<div class="form-group form-md-line-input">
								<?php
									if (!empty($data['department_id'])){
										$coresection = create_double($this->payrollemployeebonus_model->getCoreSection($data['department_id']), 'section_id', 'section_name');
				...
				<div class="portlet-body">
					<table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1">
						...
						<tbody>
							<?php
								$no=1;
								foreach ($hroemployeedata_bonus as $key=>$val){
									/*<a href='".$this->config->item('base_url').'payrollemployeebonus/editPayrollEmployeeBonus/'.$val['employee_id']."' class='btn default btn-xs purple'>
									...
									echo"
										<tr>			
											...
											<td>
												<a href='".$this->config->item('base_url').'payrollemployeebonus/addPayrollEmployeeBonus/'.$val['employee_id']."' class='btn default btn-xs green-jungle'>
