...
<script>
	...
	function reset_add_experience(){
		document.location = base_url+"HroEmployeeDataCkp/reset_add_experience";
	}

	function function_elements_add_experience(name, value){
		$.ajax({
				type: "POST",
				url : "<?php echo site_url('HroEmployeeDataCkp/function_elements_add_experience');?>",
	...
	function processAddArrayHROEmployeeExperience(){
		...
		var experience_remark				= document.getElementById("experience_remark").value;
			...
			$('#onspinspinwarehouse').css('display', 'table-row');
			  $.ajax({
			  type: "POST",
			  url : "<?php echo site_url('HroEmployeeDataCkp/processAddArrayHROEmployeeExperience');?>",
			  data: {
...
<?php 
	$year_now 	=	date('Y');
	if(!is_array($hroemployeeexperience)){
		$sesi['month']			= date('m');
		$sesi['year']			= $year_now;
	}
...
<div class="row">		
	<div class="col-md-6">
		<div class="form-group form-md-line-input">
			<input type="text" class="form-control" id="experience_company_name" name="experience_company_name" onChange="function_elements_add_experience(this.name, this.value);" value="<?php echo $data['experience_company_name'];?>">
...
<div class="row">
	<div class="col-md-6">
		<div class="form-group form-md-line-input">
			<input type="text" class="form-control" id="experience_job_title" name="experience_job_title" onChange="function_elements_add_experience(this.name, this.value);" value="<?php echo $data['experience_job_title'];?>" >
			<label>Berkerja Sebagai</label>
		</div>
	</div>	
	<div class="col-md-6">
		<div class="form-group form-md-line-input">
			<input type="text" class="form-control" id="experience_last_salary" name="experience_last_salary" onChange="function_elements_add_experience(this.name, this.value);" value="<?php echo $data['experience_last_salary'];?>">
			...
			<table class="table table-bordered table-advance table-hover">
				<thead>
					<tr>
						...
						<th style='text-align:center' width="10%">Sebagai</th>
							...
							echo"
								<tr class='odd gradeX'>
									...
									<td style='text-align  : center !important;'>
										<a href='".$this->config->item('base_url').'HroEmployeeDataCkp/deleteArrayHROEmployeeExperience/'.$val['record_id']."' onClick='javascript:return confirm(\"Apakah yakin ingin dihapus ?\")' class='btn default btn-xs red'>
