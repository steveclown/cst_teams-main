<script>
	...
	function reset_add_absence(){
		document.location = base_url+"hroemployeeattendance/reset_add_absence/<?php echo $hroemployeedata['employee_id']?>";
	}

	function function_elements_add_absence(name, value){
		$.ajax({
				type: "POST",
				url : "<?php echo site_url('hroemployeeattendance/function_elements_add_absence');?>",
...
<?php 
	echo form_open('hroemployeeattendance/processAddHROEmployeeAbsence',array('id' => 'myform', 'class' => 'horizontal-form')); 
						...
						foreach ($hroemployeeabsence as $key=>$val){
							echo"
								<tr>
									...
									<td>
										<a href='".$this->config->item('base_url').'hroemployeeattendance/deleteHROEmployeeAbsence/'.$val['employee_id']."/".$val['employee_absence_id']."' onClick='javascript:return confirm(\"Are you sure you want to delete this entry ?\")' class='btn default btn-xs red'>
