<script>
	...
	function reset_add_permit(){
		document.location = base_url+"hroemployeeattendance/reset_add_permit/<?php echo $hroemployeedata['employee_id']?>";
	}

	function function_elements_add_permit(name, value){
		$.ajax({
				type: "POST",
				url : "<?php echo site_url('hroemployeeattendance/function_elements_add_permit');?>",
...
<?php 
	echo form_open('hroemployeeattendance/processAddHROEmployeePermit',array('id' => 'myform', 'class' => 'horizontal-form')); 
						...
						foreach ($hroemployeepermit as $key=>$val){
							echo"
								<tr>
									...
									<td>
										<a href='".$this->config->item('base_url').'hroemployeeattendance/deleteHROEmployeePermit/'.$val['employee_id']."/".$val['employee_permit_id']."' onClick='javascript:return confirm(\"Are you sure you want to delete this entry ?\")' class='btn default btn-xs red'>
