<script>
	...
	function reset_add_overtime(){
		document.location = base_url+"hroemployeeattendance/reset_add_overtime/<?php echo $hroemployeedata['employee_id']?>";
	}

	function function_elements_add_overtime(name, value){
		$.ajax({
				type: "POST",
				url : "<?php echo site_url('hroemployeeattendance/function_elements_add_overtime');?>",
...
<?php 
	echo form_open('hroemployeeattendance/processAddPayrollOvertimeRequest',array('id' => 'myform', 'class' => 'horizontal-form')); 
						...
						foreach ($payrollovertimerequest as $key => $val){
							echo"
								<tr>
									...
									<td>
										<a href='".$this->config->item('base_url').'hroemployeeattendance/deletePayrollOvertimeRequest/'.$val['employee_id']."/".$val['overtime_request_id']."' onClick='javascript:return confirm(\"Are you sure you want to delete this entry ?\")' class='btn default btn-xs red'>
