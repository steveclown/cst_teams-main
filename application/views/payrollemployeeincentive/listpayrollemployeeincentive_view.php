...
<script>
    ...
    function reset_filter(){
		document.location = base_url+"payrollemployeeincentive/reset_search";
	}
	...
	$(document).ready(function(){
        $("#division_id").change(function(){
            ...
            $.ajax({
               type : "POST",
               url  : "<?php echo base_url(); ?>payrollemployeeincentive/getCoreDepartment",
    ...
    $(document).ready(function(){
        $("#department_id").change(function(){
            ...
            $.ajax({
               type : "POST",
               url  : "<?php echo base_url(); ?>payrollemployeeincentive/getCoreSection",
...
<?php 
	$data = $this->session->userdata('filter-payrollemployeeincentive',$data);
	echo form_open('payrollemployeeincentive/filter',array('id' => 'myform', 'class' => '')); 
?>
<div class="row">
	<div class="col-md-12">
							...
							<div class="form-group form-md-line-input">
								<?php
									if (!empty($data['division_id'])){
										$coredepartment = create_double($this->payrollemployeeincentive_model->getCoreDepartment($data['division_id']), 'department_id', 'department_name');
							...
							<div class="form-group form-md-line-input">
								<?php
									if (!empty($data['department_id'])){
										$coresection = create_double($this->payrollemployeeincentive_model->getCoreSection($data['department_id']), 'section_id', 'section_name');
						...
						<tbody>
							<?php
								...
								foreach ($hroemployeedata_incentive as $key=>$val){
									/*<a href='".$this->config->item('base_url').'payrollemployeeincentive/editPayrollEmployeeIncentive/'.$val['employee_id']."' class='btn default btn-xs purple'>
									...
									echo"
										<tr>			
											...
											<td>
												<a href='".$this->config->item('base_url').'payrollemployeeincentive/addPayrollEmployeeIncentive/'.$val['employee_id']."' class='btn default btn-xs green-jungle'>
