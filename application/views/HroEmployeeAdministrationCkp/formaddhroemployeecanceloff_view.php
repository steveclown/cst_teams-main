...
<script>
	...
	function reset_add_canceloff(){
		document.location = base_url+"hroemployeeadministrationckp/reset_add_canceloff/<?php echo $employee_id; ?>/<?php echo $employee_absence_date; ?>/<?php echo $employee_schedule_item_id; ?>";
	}

	function function_elements_add_permit(name, value){
		$.ajax({
				type: "POST",
				url : "<?php echo site_url('hroemployeeadministrationckp/function_elements_add_canceloff');?>",
...
<?php 
	echo form_open('hroemployeeadministrationckp/processAddHROEmployeeCancelOff',array('id' => 'myform', 'class' => 'horizontal-form')); 
						...
						foreach ($hroemployeecanceloff as $key=>$val){
							echo"
								<tr>
									...
									<td>
										<a href='".$this->config->item('base_url').'hroemployeeadministrationckp/deleteHROEmployeeCancelOff/'.$val['employee_id']."/".$val['employee_cancel_off_id']."/".$employee_absence_date."/".$employee_schedule_item_id."' onClick='javascript:return confirm(\"Are you sure you want to delete this entry ?\")' class='btn default btn-xs red'>
