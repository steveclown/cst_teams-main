<script>
    ...
    function reset_filter(){
		document.location = base_url+"payrollleaverequest/reset_search";
	}
</script>
...
?>
			...
			<h1 class="page-title">
				Employee Leave Request List <small>Manage Employee Leave Request</small>
			</h1>
			<!-- END PAGE TITLE & BREADCRUMB-->

<?php echo form_open('payrollleaverequest/filter',array('id' => 'myform', 'class' => '')); ?>
<div class="row">
	<div class="col-md-12">
						...
						<tbody>
							<?php
								...
								foreach ($hroemployeedata_leave as $key=>$val){
									/*<a href='".$this->config->item('base_url').'payrollleaverequest/editPayrollEmployeeSuspend/'.$val['employee_id']."' class='btn default btn-xs purple'>
													<i class='fa fa-edit'></i> Edit
												</a>*/
									echo"
										<tr>			
											<td>".$no."</td>						
											<td>".$this->payrollleaverequest_model->getEmployeeName($val['employee_id'])."</td>
											<td>".$this->payrollleaverequest_model->getDivisionName($val['division_id'])."</td>
											<td>".$this->payrollleaverequest_model->getDepartmentName($val['department_id'])."</td>
											<td>".$this->payrollleaverequest_model->getSectionName($val['section_id'])."</td>
											<td>
												<a href='".$this->config->item('base_url').'payrollleaverequest/addPayrollLeaveRequest/'.$val['employee_id']."' class='btn default btn-xs green-jungle'>
