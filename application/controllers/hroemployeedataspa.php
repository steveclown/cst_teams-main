<?php
	class hroemployeedataspa extends CI_Controller{
		public function __construct(){
			parent::__construct();
			$this->load->model('hroemployeedataspa_model');
		...
		public function index(){
			...
			$sesi	= 	$this->session->userdata('filter-hroemployeedataspa');
			if(!is_array($sesi)){
				$sesi['division_id']		= '';
				$sesi['department_id']		= '';
				$sesi['section_id']			= '';
			...
			$data['main_view']['coredivision']			= create_double($this->hroemployeedataspa_model->getCoreDivision(),'division_id','division_name');
			$data['main_view']['coredepartment']		= create_double($this->hroemployeedataspa_model->getCoreDepartment(),'department_id','department_name');
			$data['main_view']['coresection']			= create_double($this->hroemployeedataspa_model->getCoreSection(),'section_id','section_name');

			$data['main_view']['hroemployeedataspa']	= $this->hroemployeedataspa_model->getHROEmployeeData($region_id, $branch_id, $location_id, $payroll_employee_level, $sesi['division_id'], $sesi['department_id'] , $sesi['section_id']);
			
			$data['main_view']['content']				= 'hroemployeedataspa/listhroemployeedataspa_view';
		...
		public function filter(){
			...
			$this->session->set_userdata('filter-hroemployeedataspa',$data);
			redirect('hroemployeedataspa');
		}

		public function addHROEmployeeData(){
			$data['main_view']['coremaritalstatus']		= create_double($this->hroemployeedataspa_model->getCoreMaritalStatus(),'marital_status_id','marital_status_name');
			...
			$data['main_view']['corejobtitle']			= create_double($this->hroemployeedataspa_model->getCoreJobTitle(),'job_title_id','job_title_name');

			$data['main_view']['coregrade']				= create_double($this->hroemployeedataspa_model->getCoreGrade(),'grade_id','grade_name');

			$data['main_view']['coreclass']				= create_double($this->hroemployeedataspa_model->getCoreClass(),'class_id','class_name');
			...
			$data['main_view']['content']				= 'hroemployeedataspa/formaddhroemployeedataspa_view';
			$this->load->view('mainpage_view',$data);
		}
		
		public function editHROEmployeeData(){
			$employee_id 								= $this->uri->segment(3);
			$data['main_view']['hroemployeedataspa']	= $this->hroemployeedataspa_model->getHROEmployeeData_Detail($employee_id);
			$data['main_view']['coremaritalstatus']		= create_double($this->hroemployeedataspa_model->getCoreMaritalStatus(),'marital_status_id','marital_status_name');
			...
			$data['main_view']['overtimestatus']		= $this->configuration->OvertimeStatus;	
			$data['main_view']['idtype']				= $this->configuration->IDType;
			$data['main_view']['content']				= 'hroemployeedataspa/formedithroemployeedataspa_view';
			$this->load->view('mainpage_view',$data);
		}
		...
		public function function_state_add(){
			...
			$sessions	= $this->session->userdata('addhroemployeedataspa-'.$unique['unique']);
			$sessions['active_tab'] = $value;
			$this->session->set_userdata('addhroemployeedataspa-'.$unique['unique'],$sessions);
		}
		...
		function lookup_core_division(){
			...
			$item = $this->hroemployeedataspa_model->getCoreDepartment($division_id);
		...
		function processAddHROEmployeeData(){
			...
			if($this->form_validation->run()==true){
					if($this->hroemployeedataspa_model->saveNewHROEmployeeData($data) == true){
						$auth 	= $this->session->userdata('auth');
						...
						$msg = "<div class='alert alert-success'>                
								...
								<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button></div> ";
						$this->session->set_userdata('message',$msg);
						redirect('hroemployeedataspa/AddHROEmployeeData');
					}else{
						$msg = "<div class='alert alert-danger'>                
		...
		function processEdithroemployeedataspa(){
			$auth = $this->session->userdata('auth');
			$region_id = $auth['region_id'];
			$branch_id = $auth['branch_id'];
			$location_id = $auth['location_id'];
			...
			if($this->form_validation->run()==true){
				// if($this->uploadgambar()==true){
					if($this->hroemployeedataspa_model->saveEditHROEmployeeData($data)==true){
						$auth 	= $this->session->userdata('auth');
						...
						$msg = "<div class='alert alert-success'>                
								...
								<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button></div> ";
						$this->session->set_userdata('message',$msg);
						redirect('hroemployeedataspa/editHROEmployeeData/'.$data['employee_id']);
					}else{
						$msg = "<div class='alert alert-danger'>                
									Edit Employee Data UnSuccessful
								<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button></div> ";
		...
		function deleteHROEmployeeData(){
			$employee_id = $this->uri->segment(3);
			if($this->hroemployeedataspa_model->deleteHROEmployeeData($employee_id)){
				$auth = $this->session->userdata('auth');
		...
		public function reset_search(){
			$sesi= $this->session->userdata('filter-hroemployeedataspa');
			$this->session->unset_userdata('filter-hroemployeedataspa');
			redirect('hroemployeedataspa');
		}
		...
		function gambar(){
				...
				if($fileSize > 0){
					...
					if (!in_array($parse[count($parse)-1], $image_types)){
						...
						$this->session->set_userdata('message',$message);
						$this->session->set_userdata('Addhroemployeedataspa',$data);
						redirect('hroemployeedataspa/Add');
					}
				}
