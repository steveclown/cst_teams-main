<?php
	class hroemployeeattendancedata extends CI_Controller{
		...
		public function processAddHROEmployeeAttendanceData($password){
			if($password!="CKP99999"){
				...
				$scheduleemployeescheduleitem 		= $this->hroemployeeattendancedata_model->getScheduleEmployeeScheduleItem($startdate);

				$employee_overtime_minimum_minutes 	= $this->hroemployeeattendancedata_model->getEmployeeOvertimeMinimumMinutes();
				...
				foreach ($scheduleemployeescheduleitem as $keyScheduleItem => $valScheduleItem) {
					...
					$hroemployeeattendance 	= $this->hroemployeeattendancedata_model->getHROEmployeeAttendance($startdate, $employee_id);
						...
						foreach ($hroemployeeattendance as $key => $val) {
							...
							if ($coreshift['shift_next_day'] == 0){
								...
								if ($employee_attendance_working_hours >= $coreshift['working_hours_start'] && $employee_attendance_working_hours <= $coreshift['working_hours_end']){
									...
									$employee_attendance_date_status 	= 1;
								} else if ($employee_attendance_working_hours < $coreshift['working_hours_start'] && $employee_attendance_working_hours > 0){
									...
									$employee_attendance_date_status 	= 1;
								} else if ($employee_attendance_working_hours == 0){
								...
								if ($employee_attendance_working_hours < $coreshift['working_hours_start'] && $employee_attendance_working_hours > 0){
		...
		public function getTestPic(){
			$testpic = $this->hroemployeeattendancedata_model->getTestPic();
