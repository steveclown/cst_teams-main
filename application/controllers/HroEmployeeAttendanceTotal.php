<?php
	Class HroEmployeeAttendanceTotal extends CI_Controller{
		public function __construct(){
			parent::__construct();
			$this->load->model('MainPage_model');
			$this->load->model('HroEmployeeAttendanceTotal_model');
		...
		public function index(){
			...
			$sesi	= 	$this->session->userdata('filter-HroEmployeeAttendanceTotal');
			if(!is_array($sesi)){
				$sesi['employee_shift_id']			= '';
				$sesi['employee_monthly_period']	= '';
			}

			$data['Main_view']['scheduleemployeeshift']		= create_double($this->HroEmployeeAttendanceTotal_model->getScheduleEmployeeShift($region_id, $branch_id, $location_id), 'employee_shift_id', 'employee_shift_code');
			
			$data['Main_view']['payrollmonthlyperiod']		= create_double($this->HroEmployeeAttendanceTotal_model->getPayrollMonthlyPeriod(), 'monthly_period', 'monthly_period_date');

			$data['Main_view']['HroEmployeeAttendanceTotal']	= $this->HroEmployeeAttendanceTotal_model->getHROEmployeeAttendanceTotal($region_id, $branch_id, $location_id, $sesi['employee_shift_id'], $sesi['employee_monthly_period']);

			$data['Main_view']['content']					= 'HroEmployeeAttendanceTotal/listHroEmployeeAttendanceTotal_view';
		...
		public function filter(){
			...
			$this->session->set_userdata('filter-HroEmployeeAttendanceTotal',$data);
			redirect('HroEmployeeAttendanceTotal');
		}

		public function function_state_add(){
			$unique 	= $this->session->userdata('unique');
			$value 		= $this->input->post('value',true);
			$sessions	= $this->session->userdata('addHroEmployeeAttendanceTotal-'.$unique['unique']);
			$sessions['active_tab'] = $value;
			$this->session->set_userdata('addHroEmployeeAttendanceTotal-'.$unique['unique'],$sessions);
		}
		...
		public function reset_search(){
			$this->session->unset_userdata('filter-HroEmployeeAttendanceTotal');
			redirect('HroEmployeeAttendanceTotal');
		}

		public function reset_add(){
			$unique 	= $this->session->userdata('unique');
			$this->session->unset_userdata('addHroEmployeeAttendanceTotal-'.$unique['unique']);	
			$this->session->unset_userdata('addarrayHroEmployeeAttendanceTotalitem-'.$unique['unique']);	
			redirect('HroEmployeeAttendanceTotal/addHROEmployeeAttendanceTotal');
		}
		...
		public function addHROEmployeeAttendanceTotal(){
			...
			$data['Main_view']['content']					= 'HroEmployeeAttendanceTotal/formaddHroEmployeeAttendanceTotal_view';
			$this->load->view('MainPage_view',$data);
		}
/*
		...
		public function processCalculateHROEmployeeAttendanceTotal(){
			...
			$payrollmonthlyperiod 			= $this->HroEmployeeAttendanceTotal_model->getPayrollMonthlyPeriod_Detail($employee_monthly_period);

			$scheduleemployeeshiftitem 		= $this->HroEmployeeAttendanceTotal_model->getScheduleEmployeeShiftItem_Detail($region_id, $branch_id, $location_id, $employee_shift_id);

			$HroEmployeeAttendanceTotal 	= $this->HroEmployeeAttendanceTotal_model->getHROEmployeeAttendanceTotal_EmployeeShift($employee_shift_id, $employee_monthly_period);	
			...
			if (empty($HroEmployeeAttendanceTotal)){	
				foreach ($scheduleemployeeshiftitem as $key => $val) {
					...
					$hroemployeedata 				= $this->HroEmployeeAttendanceTotal_model->getHROEmployeeData_Detail($employee_id);

					$hroemployeeatendancedata 		= $this->HroEmployeeAttendanceTotal_model->getHROEmployeeAttendanceData_Detail($employee_id, $employee_monthly_start_date, $employee_monthly_end_date);
					...
					if (!empty($hroemployeeatendancedata)){
						...
						if ($total_permit_with_doctor_days > 1){
							$total_permit_with_doctor_payroll_days 	= 1;
							$total_permit_no_doctor_payroll_days 	= $total_permit_no_doctor_days + $total_permit_with_doctor_days - 1;
						} else {
						...
						$data_attendancetotalitem = array (
							...
							'total_permit_with_doctor_payroll_days'	=> $total_permit_with_doctor_payroll_days,
							'total_permit_no_doctor_days'			=> $total_permit_no_doctor_days,
							'total_permit_no_doctor_payroll_days'	=> $total_permit_no_doctor_payroll_days,
						...
						$dataArrayHeader	= $this->session->userdata('addarrayHroEmployeeAttendanceTotalitem-'.$unique['unique']);
						$dataArrayHeader[$data_attendancetotalitem['employee_id']] = $data_attendancetotalitem;

						$this->session->set_userdata('addarrayHroEmployeeAttendanceTotalitem-'.$unique['unique'],$dataArrayHeader);
					}
				}
			} else{
		...
		public function processAddHROEmployeeAttendanceTotal(){
			...
			$session_HroEmployeeAttendanceTotal			= $this->session->userdata('addarrayHroEmployeeAttendanceTotalitem-'.$unique['unique']);

			foreach ($session_HroEmployeeAttendanceTotal as $key => $val) {
			...
			if($this->form_validation->run()==true){
				if($this->HroEmployeeAttendanceTotal_model->insertHROEmployeeAttendanceTotal($data)){
					$employee_attendance_total_id = $this->HroEmployeeAttendanceTotal_model->getEmployeeAttendanceTotalID($data['created_id']);

					if(!empty($session_HroEmployeeAttendanceTotal)){
						foreach($session_HroEmployeeAttendanceTotal as $key => $val){
							$data_HroEmployeeAttendanceTotalitem = array(
							...
							$this->HroEmployeeAttendanceTotal_model->insertHROEmployeeAttendanceTotalItem($data_HroEmployeeAttendanceTotalitem);
						}
					}
		...
		public function showdetail(){
			...
			$data['Main_view']['HroEmployeeAttendanceTotal']		= $this->HroEmployeeAttendanceTotal_model->getHROEmployeeAttendanceTotal_Detail($employee_attendance_total_id);

			$data['Main_view']['HroEmployeeAttendanceTotalitem']	= $this->HroEmployeeAttendanceTotal_model->getHROEmployeeAttendanceTotalItem_Detail($employee_attendance_total_id);
			
			$data['Main_view']['employeestatus']					= $this->configuration->EmployeeStatus();

			
			$data['Main_view']['content']							= 'HroEmployeeAttendanceTotal/formdetailHroEmployeeAttendanceTotal_view';
		...
		public function exportHROEmployeeAttendanceTotal(){
			...
			$HroEmployeeAttendanceTotal			= $this->HroEmployeeAttendanceTotal_model->getHROEmployeeAttendanceTotal_Detail($employee_attendance_total_id);

			$HroEmployeeAttendanceTotalitem		= $this->HroEmployeeAttendanceTotal_model->getHROEmployeeAttendanceTotalItem_Detail($employee_attendance_total_id);
			
			if(!empty($HroEmployeeAttendanceTotalitem)){
				...
				$this->excel->getActiveSheet()->mergeCells("B1:AL1");
				...
				$this->excel->getActiveSheet()->getStyle('B3:AL3')->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
				$this->excel->getActiveSheet()->getStyle('B3:AL3')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
				$this->excel->getActiveSheet()->getStyle('B3:AL3')->getFont()->setBold(true);
				...
				$this->excel->getActiveSheet()->setCellValue('J3',"Monthly Period");	
				$this->excel->getActiveSheet()->setCellValue('K3',"Monthly Period Start Date");
				$this->excel->getActiveSheet()->setCellValue('L3',"Monthly Period End Date");
				$this->excel->getActiveSheet()->setCellValue('M3',"Employee Status");
				$this->excel->getActiveSheet()->setCellValue('N3',"Employee Hire Date");
				$this->excel->getActiveSheet()->setCellValue('O3',"Employee Working Months");
				$this->excel->getActiveSheet()->setCellValue('P3',"Total Working Days");
				$this->excel->getActiveSheet()->setCellValue('Q3',"Total Working Payroll Days");
				$this->excel->getActiveSheet()->setCellValue('R3',"Total Off Payroll Days");
				$this->excel->getActiveSheet()->setCellValue('S3',"Total Default Payroll Days");
				$this->excel->getActiveSheet()->setCellValue('T3',"Total Permit With Doctor Days");
				$this->excel->getActiveSheet()->setCellValue('U3',"Total Permit With Doctor Payroll Days");
				$this->excel->getActiveSheet()->setCellValue('V3',"Total Permit No Doctor Days");	
				$this->excel->getActiveSheet()->setCellValue('W3',"Total Permit No Doctor Payroll Days");
				$this->excel->getActiveSheet()->setCellValue('X3',"Total Absence Payroll Days");
				$this->excel->getActiveSheet()->setCellValue('Y3',"Total Cancel Off Payroll Days");	
				$this->excel->getActiveSheet()->setCellValue('Z3',"Total Swap Off Payroll Days");
				$this->excel->getActiveSheet()->setCellValue('AA3',"Total Early Days");
				$this->excel->getActiveSheet()->setCellValue('AB3',"Total Home Early Payroll Less 1 Hours ( Days )");
				$this->excel->getActiveSheet()->setCellValue('AC3',"Total Home Early Payroll Less 5 Hours ( Days )");
				$this->excel->getActiveSheet()->setCellValue('AD3',"Total Home Early Payroll More 5 Hours ( Days )");
				$this->excel->getActiveSheet()->setCellValue('AE3',"Total Home Early Hours");
				$this->excel->getActiveSheet()->setCellValue('AF3',"Total Late Days");
				$this->excel->getActiveSheet()->setCellValue('AG3',"Total Late Hours");
				$this->excel->getActiveSheet()->setCellValue('AH3',"Total Late Minutes");
				$this->excel->getActiveSheet()->setCellValue('AI3',"Total Overtime Days");
				$this->excel->getActiveSheet()->setCellValue('AJ3',"Total Overtime Hours");
				$this->excel->getActiveSheet()->setCellValue('AK3',"Total Overtime Minutes");
				$this->excel->getActiveSheet()->setCellValue('AL3',"Total Overtime Hours");
				...
				foreach($HroEmployeeAttendanceTotalitem as $key=>$val){
					if(is_numeric($key)){
						...
						$this->excel->getActiveSheet()->getStyle('AH'.$j)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
						$this->excel->getActiveSheet()->getStyle('AI'.$j)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
						$this->excel->getActiveSheet()->getStyle('AJ'.$j)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
						$this->excel->getActiveSheet()->getStyle('AK'.$j)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
						$this->excel->getActiveSheet()->getStyle('AL'.$j)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

						
						$this->excel->getActiveSheet()->setCellValue('B'.$j, $no);
						$this->excel->getActiveSheet()->setCellValue('C'.$j, $HroEmployeeAttendanceTotal['employee_shift_code']);
						...
						$this->excel->getActiveSheet()->setCellValue('J'.$j, $val['employee_monthly_period']);
						$this->excel->getActiveSheet()->setCellValue('K'.$j, $val['employee_monthly_start_date']);
						$this->excel->getActiveSheet()->setCellValue('L'.$j, $val['employee_monthly_end_date']);
						$this->excel->getActiveSheet()->setCellValue('M'.$j, $this->configuration->EmployeeStatus($val['employee_employment_status']));
						$this->excel->getActiveSheet()->setCellValue('N'.$j, $val['employee_hire_date']);
						$this->excel->getActiveSheet()->setCellValue('O'.$j, number_format($val['employee_working_months'], 2));
						$this->excel->getActiveSheet()->setCellValue('P'.$j, $val['total_working_days']);
						$this->excel->getActiveSheet()->setCellValue('Q'.$j, $val['total_working_payroll_days']);
						$this->excel->getActiveSheet()->setCellValue('R'.$j, $val['total_working_off_payroll_days']);
						$this->excel->getActiveSheet()->setCellValue('S'.$j, $val['total_default_payroll_days']);
						$this->excel->getActiveSheet()->setCellValue('T'.$j, $val['total_permit_with_doctor_days']);
						$this->excel->getActiveSheet()->setCellValue('U'.$j, $val['total_permit_with_doctor_payroll_days']);
						$this->excel->getActiveSheet()->setCellValue('V'.$j, $val['total_permit_no_doctor_days']);
						$this->excel->getActiveSheet()->setCellValue('W'.$j, $val['total_permit_no_doctor_payroll_days']);
						$this->excel->getActiveSheet()->setCellValue('X'.$j, $val['total_absence_payroll_days']);
						$this->excel->getActiveSheet()->setCellValue('Y'.$j, $val['total_cancel_off_payrol_days']);
						$this->excel->getActiveSheet()->setCellValue('Z'.$j, $val['total_swap_off_payroll_days']);
						$this->excel->getActiveSheet()->setCellValue('AA'.$j, $val['total_early_days']);
						$this->excel->getActiveSheet()->setCellValue('AB'.$j, $val['total_early_payroll_less_1_days']);
						$this->excel->getActiveSheet()->setCellValue('AC'.$j, $val['total_early_payroll_less_5_days']);
						$this->excel->getActiveSheet()->setCellValue('AD'.$j, $val['total_early_payroll_more_5_days']);
						$this->excel->getActiveSheet()->setCellValue('AE'.$j, $val['total_early_hours_list']);
						$this->excel->getActiveSheet()->setCellValue('AF'.$j, $val['total_late_days']);
						$this->excel->getActiveSheet()->setCellValue('AG'.$j, $val['total_late_hours']);
						$this->excel->getActiveSheet()->setCellValue('AH'.$j, $val['total_late_minutes']);
						$this->excel->getActiveSheet()->setCellValue('AI'.$j, $val['total_overtime_days']);
						$this->excel->getActiveSheet()->setCellValue('AJ'.$j, $val['total_overtime_hours']);
						$this->excel->getActiveSheet()->setCellValue('AK'.$j, $val['total_overtime_minutes']);
						$this->excel->getActiveSheet()->setCellValue('AL'.$j, $val['total_overtime_hours_list']);
				...
				$filename='Employee_Attendance_Total_'.$HroEmployeeAttendanceTotal['employee_shift_code'].'_'.$employee_monthly_period.'_'.$employee_monthly_start_date.'_'.$employee_monthly_end_date.'.xls';
