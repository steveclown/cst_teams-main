<?php
	Class PayrollEmployeeDatackp extends CI_Controller{
		public function __construct(){
			parent::__construct();
			$this->load->model('MainPage_model');
			$this->load->model('PayrollEmployeeDatackp_model');
		...
		public function index(){
			...
			$sesi	= 	$this->session->userdata('filter-PayrollEmployeeDatackp');
			if(!is_array($sesi)){
				$sesi['division_id']		= '';
				$sesi['department_id']		= '';
				$sesi['section_id']			= '';
			...
			$data['Main_view']['coredivision']				= create_double($this->PayrollEmployeeDatackp_model->getCoreDivision(),'division_id','division_name');

			$data['Main_view']['hroemployeedata_daily']		= $this->PayrollEmployeeDatackp_model->getHROEmployeeData_Daily($region_id, $branch_id, $location_id, $sesi['division_id'], $sesi['department_id'] , $sesi['section_id']);

			$data['Main_view']['content']					= 'PayrollEmployeeDatackp/ListPayrollEmployeeDatackp_view';
		...
		public function filter(){
			...
			$this->session->set_userdata('filter-PayrollEmployeeDatackp', $data);
			redirect('PayrollEmployeeDatackp');
		}
		...
		public function getCoreDepartment(){
			...
			$item = $this->PayrollEmployeeDatackp_model->getCoreDepartment($division_id);
		...
		public function getCoreSection(){
			...
			$item = $this->PayrollEmployeeDatackp_model->getCoreSection($department_id);
			$data .= "<option value=''>--Choose One--</option>";
			foreach ($item as $mp){
		...
		public function reset_search(){
			$sesi= $this->session->userdata('filter-PayrollEmployeeDatackp');
			$this->session->unset_userdata('filter-PayrollEmployeeDatackp');
			redirect('PayrollEmployeeDatackp');
		}
		...
		public function function_state_add(){
			...
			$sessions	= $this->session->userdata('addpayrollemployeedatackp-'.$unique['unique']);
			$sessions['active_tab'] = $value;
			$this->session->set_userdata('addpayrollemployeedatackp-'.$unique['unique'],$sessions);
		}
		...
		public function reset_add(){
			$unique 	= $this->session->userdata('unique');
			$this->session->unset_userdata('addpayrollemployeedatackp-'.$unique['unique']);	
			redirect('PayrollEmployeeDatackp');
		}
		...
		public function addPayrollEmployeeData(){
			...
			$data['Main_view']['hroemployeedata']					= $this->PayrollEmployeeDatackp_model->getHROEmployeeData_Detail($employee_id);

			$data['Main_view']['corebank']							= create_double($this->PayrollEmployeeDatackp_model->getCoreBank(),'bank_id','bank_name');

			$data['Main_view']['coreallowance']						= create_double($this->PayrollEmployeeDatackp_model->getCoreAllowance(),'allowance_id', 'allowance_name');

			$data['Main_view']['corededuction']						= create_double($this->PayrollEmployeeDatackp_model->getCoreDeduction(),'deduction_id','deduction_name');

			$data['Main_view']['corepremiattendance']				= create_double($this->PayrollEmployeeDatackp_model->getCorePremiAttendance(),'premi_attendance_id','premi_attendance_name');

			$data['Main_view']['bpjsstatus']						= $this->configuration->BPJSStatus();

			$data['Main_view']['coreloantype']						= create_double($this->PayrollEmployeeDatackp_model->getCoreLoanType(),'loan_type_id','loan_type_name');

			$data['Main_view']['monthlist']							= $this->configuration->Month();

			$data['Main_view']['payrollemployeepayment']			= $this->PayrollEmployeeDatackp_model->getPayrollEmployeePayment_Detail($employee_id);

			$data['Main_view']['payrollemployeeallowance']			= $this->PayrollEmployeeDatackp_model->getPayrollEmployeeAllowance_Detail($employee_id);

			$data['Main_view']['payrollemployeededuction']			= $this->PayrollEmployeeDatackp_model->getPayrollEmployeeDeduction_Detail($employee_id);

			$data['Main_view']['payrollemployeepremiattendance']	= $this->PayrollEmployeeDatackp_model->getPayrollEmployeePremiAttendance_Detail($employee_id);

			$data['Main_view']['payrollemployeebpjs']				= $this->PayrollEmployeeDatackp_model->getPayrollEmployeeBPJS_Detail($employee_id);

			$data['Main_view']['payrollemployeeloan']				= $this->PayrollEmployeeDatackp_model->getPayrollEmployeeLoan_Detail($employee_id);

			$data['Main_view']['content']							= 'PayrollEmployeeDatackp/FormAddPayrollEmployeeDatackp_view';
			$this->load->view('MainPage_view',$data);
		}
		...
		public function reset_add_payment(){
			...
			redirect('PayrollEmployeeDatackp/addPayrollEmployeeData/'.$employee_id);
		}
		...
		public function processAddPayrollEmployeePayment(){
			...
			if($this->form_validation->run()==true){
				if($this->PayrollEmployeeDatackp_model->checkPaymentBankAcctNo($data['payment_bank_acct_no'])){
					if($this->PayrollEmployeeDatackp_model->insertPayrollEmployeePayment($data)){
						$auth = $this->session->userdata('auth');
						...
						$this->session->unset_userdata('addpayrollemployeepayment-'.$unique['unique']);	
						redirect('PayrollEmployeeDatackp/addPayrollEmployeeData/'.$data['employee_id']);
					}else{
		...
		public function deletePayrollEmployeePayment(){
			...
			if($this->PayrollEmployeeDatackp_model->deletePayrollEmployeePayment($data)){
				$auth = $this->session->userdata('auth');
		...
		public function processAddPayrollEmployeeAllowance(){
			...
			if($this->form_validation->run()==true){
				if($this->PayrollEmployeeDatackp_model->insertPayrollEmployeeAllowance($data)){
					$auth = $this->session->userdata('auth');
		...
		public function deletePayrollEmployeeAllowance(){
			...
			if($this->PayrollEmployeeDatackp_model->deletePayrollEmployeeAllowance($data)){
				$auth = $this->session->userdata('auth');
		...
		public function processAddPayrollEmployeeDeduction(){
			...
			if($this->form_validation->run()==true){
				if($this->PayrollEmployeeDatackp_model->insertPayrollEmployeeDeduction($data)){
					$auth = $this->session->userdata('auth');
		...
		public function deletePayrollEmployeeDeduction(){
			...
			if($this->PayrollEmployeeDatackp_model->deletePayrollEmployeeDeduction($data)){
				$auth = $this->session->userdata('auth');
		...
		public function processAddPayrollEmployeePremiAttendance(){
			...
			if($this->form_validation->run()==true){
				if($this->PayrollEmployeeDatackp_model->insertPayrollEmployeePremiAttendance($data)){
					$auth = $this->session->userdata('auth');
		...
		public function deletePayrollEmployeePremiAttendance(){
			...
			if($this->PayrollEmployeeDatackp_model->deletePayrollEmployeePremiAttendance($data)){
				$auth = $this->session->userdata('auth');
		...
		public function processAddPayrollEmployeeBPJS(){
			...
			if($this->form_validation->run()==true){
				if($this->PayrollEmployeeDatackp_model->insertPayrollEmployeeBPJS($data)){
					$auth = $this->session->userdata('auth');
		...
		public function processOutPayrollEmployeeBPJS(){
			$auth 		= $this->session->userdata('auth');
			$unique 	= $this->session->userdata('unique');

			$data = array(
				'employee_id' 				=> $this->input->post('employee_id',true),
				'employee_bpjs_id'			=> $this->input->post('employee_bpjs_id_out',true),
				'bpjs_out_status_remark'	=> $this->input->post('bpjs_out_status_remark',true),
				'bpjs_out_status' 			=> 1,
				'bpjs_out_date' 			=> date("Y-m-d"),
			...
			$this->form_validation->set_rules('employee_id', 'Employee Name', 'required');
			$this->form_validation->set_rules('bpjs_out_status_remark', 'BPJS Out Remark', 'required');

			if($this->form_validation->run()==true){
				if($this->PayrollEmployeeDatackp_model->updateOutPayrollEmployeeBPJS($data)){
					$this->fungsi->set_log($auth['user_id'], '1003','Application.PayrollOnOutBPJS.processOutPayrollOnOutBPJS',$auth['user_id'], 'Update Payroll On Out BPJS');
		...
		public function processAddPayrollEmployeeLoan(){
			...
			if($this->form_validation->run()==true){
				if($this->PayrollEmployeeDatackp_model->insertPayrollEmployeeLoan($data)){
					$auth = $this->session->userdata('auth');

					$employee_loan_id = $this->PayrollEmployeeDatackp_model->getEmployeeLoanID($data['created_id']);
					...
					for($i=1; $i<=$count; $i++){
						...
						$this->PayrollEmployeeDatackp_model->insertPayrollEmployeeLoanItem($data_item);
					}
		...
		public function deletePayrollEmployeeLoan(){
			...
			if($this->PayrollEmployeeDatackp_model->deletePayrollEmployeeLoan($data)){
				$auth = $this->session->userdata('auth');
				$this->fungsi->set_log($auth['user_id'],'1005','Application.PayrollEmployeeAllowance_Data.delete',$auth['user_id'],'Delete Employee Allowance');
				$msg = "<div class='alert alert-success'>                
