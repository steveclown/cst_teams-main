<?php
class payrollmonthlyperiod extends MY_Controller
{
	public function __construct()
	{
		parent::__construct();

		$menu = 'payroll-monthly-period';
	...
	public function reset_data()
	{
		...
		redirect('payroll-monthly-period/add');
	}
		...
		if ($this->form_validation->run() == true) {
			if ($this->payrollmonthlyperiod_model->saveEditPayrollMonthlyPeriod($data) == true) {
				...
				$msg = "<div class='alert alert-success'>                
							...
							<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button></div> ";
				$this->session->set_userdata('message', $msg);
				redirect('payroll-monthly-period/editPayrollMonthlyPeriod/' . $data['training_job_title_id']);
			} else {
				$msg = "<div class='alert alert-danger'>                
	...
	public function deletePayrollMonthlyPeriod()
	{
		if ($this->payrollmonthlyperiod_model->deletePayrollMonthlyPeriod($this->uri->segment(3))) {
			...
			$msg = "<div class='alert alert-success'>                
						...
						<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button></div> ";
			$this->session->set_userdata('message', $msg);
			redirect('payroll-monthly-period');
		} else {
			$msg = "<div class='alert alert-danger'>                
