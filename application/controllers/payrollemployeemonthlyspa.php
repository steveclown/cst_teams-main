<?php
	Class payrollemployeemonthlyspa extends CI_Controller{
		public function __construct(){
			parent::__construct();
			$this->load->model('payrollemployeemonthlyspa_model');
		...
		public function index(){
			...
			$sesi	= 	$this->session->userdata('filter-payrollemployeemonthlyspa');
			if(!is_array($sesi)){
				$sesi['division_id']		= '';
				$sesi['department_id']		= '';
				$sesi['section_id']			= '';
			...
			$data['main_view']['coredivision']				= create_double($this->payrollemployeemonthlyspa_model->getCoreDivision(),'division_id','division_name');
			$data['main_view']['coredepartment']			= create_double($this->payrollemployeemonthlyspa_model->getCoreDepartment(),'department_id','department_name');
			$data['main_view']['coresection']				= create_double($this->payrollemployeemonthlyspa_model->getCoreSection(),'section_id','section_name');
			$data['main_view']['hroemployeedata']			= create_double($this->payrollemployeemonthlyspa_model->getHROEmployeeData_Filter($region_id, $branch_id, $location_id, $payroll_employee_level),'employee_id','employee_name');

			$data['main_view']['hroemployeedata_monthly']	= $this->payrollemployeemonthlyspa_model->getHROEmployeeData_Monthly($region_id, $branch_id, $location_id, $payroll_employee_level, $sesi['employee_id'], $sesi['division_id'], $sesi['department_id'] , $sesi['section_id']);

			$data['main_view']['content']					= 'payrollemployeemonthlyspa/listpayrollemployeemonthlyspa_view';
		...
		public function filter(){
			...
			$this->session->set_userdata('filter-payrollemployeemonthlyspa',$data);
			redirect('payrollemployeemonthlyspa');
		}

		public function function_state_add(){
			$unique 	= $this->session->userdata('unique');
			$value 		= $this->input->post('value',true);
			$sessions	= $this->session->userdata('addpayrollemployeemonthlyspa-'.$unique['unique']);
			$sessions['active_tab'] = $value;
			$this->session->set_userdata('addpayrollemployeemonthlyspa-'.$unique['unique'],$sessions);
		}
		...
		public function reset_search(){
			$sesi= $this->session->userdata('filter-payrollemployeemonthlyspa');
			$this->session->unset_userdata('filter-payrollemployeemonthlyspa');
			redirect('payrollemployeemonthlyspa');
		}

		public function reset_session(){
			$sesi 	= $this->session->userdata('unique');
			$this->session->unset_userdata('addpayrollemployeemonthlyspa-'.$sesi['unique']);	
		...
		public function addPayrollEmployeeMonthly(){
			...
			$employee_hire_date 				= $this->payrollemployeemonthlyspa_model->getEmployeeHireDate($employee_id);

			$payrollmonthlyperiod 				= $this->payrollemployeemonthlyspa_model->getPayrollMonthlyPeriod();
			...
			/*Length of Service*/
			$payrollemployeelengthservice 		= $this->payrollemployeemonthlyspa_model->getPayrollEmployeeLengthService($employee_id, $length_of_service_month, $year_period);

			$employee_length_service_amount 	= $payrollemployeelengthservice['employee_length_service_amount'];

			/*Payroll Employee Incentive*/
			$employee_incentive_amount 			= $this->payrollemployeemonthlyspa_model->getPayrollEmployeeIncentive($employee_id, $year_period);

			/*Payroll Employee Loan*/
			$employee_loan_amount 				= $this->payrollemployeemonthlyspa_model->getPayrollEmployeeLoan($employee_id, $monthly_period);

			$payrollemployeepayment 			= $this->payrollemployeemonthlyspa_model->getPayrollEmployeePayment($employee_id, $year_period);

			/*Leave Calculation*/
			$payrollleaverequest 				= $this->payrollemployeemonthlyspa_model->getPayrollLeaveRequest($employee_id, $payrollmonthlyperiod['monthly_period_start_date'], $payrollmonthlyperiod['monthly_period_end_date']);

			$leave_request_days 				= count($payrollleaverequest);

			/*Day Off*/
			$hroemployeeworkingdayoff 			= $this->payrollemployeemonthlyspa_model->getHROmployeeWorkingDayOff($employee_id, $payrollmonthlyperiod['monthly_period_start_date'], $payrollmonthlyperiod['monthly_period_end_date']);

			$working_dayoff_days 				= count($hroemployeeworkingdayoff);

			/*Overtime*/
			$payrollovertimerequest 			= $this->payrollemployeemonthlyspa_model->getPayrollOvertimeRequest($employee_id, $payrollmonthlyperiod['monthly_period_start_date'], $payrollmonthlyperiod['monthly_period_end_date']);

			/*Home Early*/
			$hroemployeehomeearlymonthly 		= $this->payrollemployeemonthlyspa_model->getHROEmployeeHomeEarlyMonthly($employee_id, $payrollmonthlyperiod['monthly_period_start_date'], $payrollmonthlyperiod['monthly_period_end_date']);

			$payrollemployeeallowance 			= $this->payrollemployeemonthlyspa_model->getPayrollEmployeeAllowance($employee_id, $year_period);

			$employee_premi_attendance_amount 	= $this->payrollemployeemonthlyspa_model->getEmployeePremiAttendanceAmount($employee_id, $year_period);
			...
			$corelate = $this->payrollemployeemonthlyspa_model->getCoreLate();
			...
			foreach($corelate as $keyCoreLate => $valCoreLate){
				...
				$hroemployeelate 		= $this->payrollemployeemonthlyspa_model->getHROEmployeeLate($employee_id, $payrollmonthlyperiod['monthly_period_start_date'], $payrollmonthlyperiod['monthly_period_end_date'], $late_id);
			...
			$coreabsence 					= $this->payrollemployeemonthlyspa_model->getCoreAbsence();
			...
			foreach($coreabsence as $keyCoreAbsence => $valCoreAbsence){
				...
				$hroemployeeabsence 		= $this->payrollemployeemonthlyspa_model->getHROEmployeeAbsence($employee_id, $payrollmonthlyperiod['monthly_period_start_date'], $payrollmonthlyperiod['monthly_period_end_date'], $absence_id);
			...
			$corehomeearly = $this->payrollemployeemonthlyspa_model->getCoreHomeEarly();
			...
			foreach($corehomeearly as $keyCoreHomeEarly => $valCoreHomeEarly){
				...
				$hroemployeehomeearly 		= $this->payrollemployeemonthlyspa_model->getHROEmployeeHomeEarly($employee_id, $payrollmonthlyperiod['monthly_period_start_date'], $payrollmonthlyperiod['monthly_period_end_date'], $home_early_id);
				...
				if ($employee_home_early_days > 0){
					...
					$sesi 	= $this->session->userdata('unique');
					$data_payrollemployeehomeearly = $this->session->userdata('addpayrollemployeemonthlyspa-'.$sesi['unique']);

					$this->session->set_userdata('addpayrollemployeemonthlyspa-'.$sesi['unique'],$data_payrollemployeehomeearly);
				}
			}
			...
			foreach ($payrollemployeeallowance as $keyAllowance=>$valAllowance){
				...
				$sesi 	= $this->session->userdata('unique');
				$data_payrollemployeeallowance = $this->session->userdata('addpayrollemployeemonthlyspa-'.$sesi['unique']);

				$this->session->set_userdata('addpayrollemployeemonthlyspa-'.$sesi['unique'],$data_payrollemployeeallowance);
			}
			...
			$overtime_request_amount = $this->payrollemployeemonthlyspa_model->getPaymentBasicOvertime($employee_id, $year_period);

			$coreovertimetype = $this->payrollemployeemonthlyspa_model->getCoreOvertimeType();
			...
			foreach($payrollovertimerequest as $keyOvertime=>$valOvertime){
				...
				$dayoff_date = $this->payrollemployeemonthlyspa_model->getDayOffDate($overtime_request_date);
			...
			if ($employee_overtime_amount_total > 0){
				...
				$sesi 	= $this->session->userdata('unique');
				$data_payrollemployeeovertime = $this->session->userdata('addpayrollemployeemonthlyspa-'.$sesi['unique']);
				$this->session->set_userdata('addpayrollemployeemonthlyspa-'.$sesi['unique'],$data_payrollemployeeovertime);
			}

			/*exit;*/
			
			$payrollemployeebpjs			= $this->payrollemployeemonthlyspa_model->getPayrollEmployeeBPJS($employee_id);

			$payrollemployeepremiattendance = $this->payrollemployeemonthlyspa_model->getPayrollEmployeePremiAttendance($employee_id, $year_period);


			## Calculate Assignment Business Trip
			$assignmentbusinesstrip = $this->payrollemployeemonthlyspa_model->getAssignmentBusinessTrip($employee_id, $payrollmonthlyperiod['monthly_period_start_date'], $payrollmonthlyperiod['monthly_period_end_date']);
			...
			if (!empty($data_payrollemployeemonthlyspadeduction)){
			...
			exit;
*/
			$data_payrollemployeemonthlyspa = array(
			...
			foreach ($payrollleaverequest as $keyLeaveRequest=>$valLeaveRequest){
				...
				$sesi 	= $this->session->userdata('unique');
				$data_payrollleaverequest = $this->session->userdata('addpayrollemployeemonthlyspa-'.$sesi['unique']);

				$this->session->set_userdata('addpayrollemployeemonthlyspa-'.$sesi['unique'],$data_payrollleaverequest);
			}
			...
			foreach ($hroemployeeabsence as $keyAbsence=>$valAbsence){
				...
				$sesi 	= $this->session->userdata('unique');
				$data_payrollemployeeabsence = $this->session->userdata('addpayrollemployeemonthlyspa-'.$sesi['unique']);

				$this->session->set_userdata('addpayrollemployeemonthlyspa-'.$sesi['unique'],$data_payrollemployeeabsence);
			}
			...
			foreach ($hroemployeepermit as $keyPermit=>$valPermit){
				...
				$sesi 	= $this->session->userdata('unique');
				$data_payrollemployeepermit = $this->session->userdata('addpayrollemployeemonthlyspa-'.$sesi['unique']);

				$this->session->set_userdata('addpayrollemployeemonthlyspa-'.$sesi['unique'],$data_payrollemployeepermit);
			}
			...
			foreach ($hroemployeeworkingdayoff as $keyDayOff=>$valDayOff){
				...
				$sesi 	= $this->session->userdata('unique');
				$data_payrollemployeedayoff = $this->session->userdata('addpayrollemployeemonthlyspa-'.$sesi['unique']);

				$this->session->set_userdata('addpayrollemployeemonthlyspa-'.$sesi['unique'],$data_payrollemployeedayoff);
			}
			...
			foreach ($payrollovertimerequest as $keyOvertime=>$valOvertime){
				...
				$sesi 	= $this->session->userdata('unique');
				$data_payrollemployeeovertimerequest = $this->session->userdata('addpayrollemployeemonthlyspa-'.$sesi['unique']);

				$this->session->set_userdata('addpayrollemployeemonthlyspa-'.$sesi['unique'],$data_payrollemployeeovertimerequest);
			}
			...
			foreach ($hroemployeelate as $keyLate=>$valLate){
				...
				$sesi 	= $this->session->userdata('unique');
				$data_payrollemployeelate = $this->session->userdata('addpayrollemployeemonthlyspa-'.$sesi['unique']);

				$this->session->set_userdata('addpayrollemployeemonthlyspa-'.$sesi['unique'],$data_payrollemployeelate);
			}
			...
			foreach ($hroemployeehomeearly as $keyHomeEarly=>$valHomeEarly){
				...
				$sesi 	= $this->session->userdata('unique');
				$data_payrollemployeehomeearlymonthly = $this->session->userdata('addpayrollemployeemonthlyspa-'.$sesi['unique']);

				$this->session->set_userdata('addpayrollemployeemonthlyspa-'.$sesi['unique'],$data_payrollemployeehomeearlymonthly);
			}
			...
			foreach ($assignmentbusinesstrip as $keyBusinessTrip => $valBusinessTrip){
				$data_payrollemployeemonthlyspabusinesstrip = array(
				...
				$dataArrayHeader[$data_payrollemployeemonthlyspabusinesstrip['business_trip_id']] = $data_payrollemployeemonthlyspabusinesstrip;

				$this->session->set_userdata('addarrayemployeemonthlybusinesstrip-'.$unique['unique'],$dataArrayHeader);
				$sesi 	= $this->session->userdata('unique');
				$data_payrollemployeemonthlyspabusinesstrip = $this->session->userdata('addpayrollemployeemonthlyspa-'.$sesi['unique']);

				$this->session->set_userdata('addpayrollemployeemonthlyspa-'.$sesi['unique'],$data_payrollemployeemonthlyspabusinesstrip);
			}

			/*exit;*/
			$data['main_view']['hroemployeedata']						= $this->payrollemployeemonthlyspa_model->getHROEmployeeData($employee_id);
			$data['main_view']['payrollemployeemonthlyspa_data']		= $this->payrollemployeemonthlyspa_model->getPayrollEmployeeMonthly_Data($employee_id);
			
			$data['main_view']['payrollemployeepayment']				= $payrollemployeepayment;
			$data['main_view']['payrollmonthlyperiod']					= $payrollmonthlyperiod;
			$data['main_view']['payrollemployeemonthlyspa']				= $data_payrollemployeemonthlyspa;

			$data['main_view']['content']								= 'payrollemployeemonthlyspa/listaddpayrollemployeemonthlyspa_view';
			$this->load->view('mainpage_view',$data);
		}
		...
		public function processCalculatePayrollEmployeeMonthly(){
			...
			$calculate_working_days 					= $this->input->post('employee_monthly_working_days',true);
			...
			$employee_monthly_working_days = $calculate_working_days + $working_dayoff_days - $leave_request_days - $total_employee_absence_days - ($employee_home_early_days * 0.5) - $total_employee_permit_days;
...
*/
		...
		public function processAddPayrollEmployeeMonthly(){
			...
			$data = array(
				...
				'employee_monthly_business_trip_amount'			=> $this->input->post('employee_monthly_business_trip_amount',true),
			...
			$session_businesstrip		= $this->session->userdata('addarrayemployeemonthlybusinesstrip-'.$unique['unique']);
			...
			if($this->form_validation->run()==true){
				if($this->payrollemployeemonthlyspa_model->saveNewPayrollEmployeeMonthly($data)){
					$employee_monthly_id = $this->payrollemployeemonthlyspa_model->getEmployeeMonthlyID($data['created_on'], $data['created_id']);
					...
					if(!empty($session_home_early)){
						foreach($session_home_early as $key=>$val){
							...
							$this->payrollemployeemonthlyspa_model->saveNewPayrollEmployeeMonthlyEarly($data_home_early);
						}
					}
					...
					if(!empty($session_overtime)){
						foreach($session_overtime as $key=>$val){
							...
							$this->payrollemployeemonthlyspa_model->saveNewPayrollEmployeeMonthlyOvertime($data_overtime);
						}
					}
					...
					if(!empty($session_deduction)){
						foreach($session_deduction as $key=>$val){
							...
							$this->payrollemployeemonthlyspa_model->saveNewPayrollEmployeeMonthlyDeduction($data_deduction);
						}
					}
					...
					if(!empty($session_allowance)){
						foreach($session_allowance as $key=>$val){
							...
							$this->payrollemployeemonthlyspa_model->saveNewPayrollEmployeeMonthlyAllowance($data_allowance);
						}
					}
					...
					if(!empty($session_leaverequest)){
						foreach($session_leaverequest as $key=>$val){
							$data_leaverequest = array(
								...
								'leave_request_detail_date'			=> tgltodb($val['leave_request_detail_date']),
								'leave_request_description'			=> $val['leave_request_description'],
								'data_state'						=> 0,
								'created_id'						=> $data['created_id'],
								'created_on'						=> $data['created_on']
							);
							$this->payrollemployeemonthlyspa_model->saveNewPayrollEmployeeMonthlyLeave($data_leaverequest);
						}
					}
					...
					if(!empty($session_dayoff)){
						foreach($session_dayoff as $key=>$val){
							$data_dayoff = array(
								...
								'working_dayoff_detail_date'			=> tgltodb($val['working_dayoff_detail_date']),
								'data_state'							=> 0,
								'created_id'							=> $data['created_id'],
								'created_on'							=> $data['created_on']
							);
							$this->payrollemployeemonthlyspa_model->saveNewPayrollEmployeeMonthlyDayOff($data_dayoff);
						}
					}	
					...
					if(!empty($session_overtimerequest)){
						foreach($session_overtimerequest as $key=>$val){
							$data_overtimerequest = array(
								...
								'overtime_request_date'			=> tgltodb($val['overtime_request_date']),
								'overtime_request_duration'		=> $val['overtime_request_duration'],
								'data_state'					=> 0,
								'created_id'					=> $data['created_id'],
								'created_on'					=> $data['created_on']
							);
							$this->payrollemployeemonthlyspa_model->saveNewPayrollEmployeeMonthlyOvertimeRequest($data_overtimerequest);
						}
					}
					...
					if(!empty($session_homeearlymonthly)){
						foreach($session_homeearlymonthly as $key=>$val){
							$data_homeearlymonthly = array(
								...
								'employee_home_early_monthly_date'			=> tgltodb($val['employee_home_early_monthly_date']),
								'employee_home_early_monthly_hour'			=> $val['employee_home_early_monthly_hour'],
								'data_state'								=> 0,
								'created_id'								=> $data['created_id'],
								'created_on'								=> $data['created_on']
							);
							$this->payrollemployeemonthlyspa_model->saveNewPayrollEmployeeMonthlyHomeEarly($data_homeearlymonthly);
						}
					}
					...
					if(!empty($session_permit)){
						foreach($session_permit as $key=>$val){
							...
							$this->payrollemployeemonthlyspa_model->saveNewPayrollEmployeeMonthlyPermit($data_permit);
						}
					}
					...
					if(!empty($session_absence)){
						foreach($session_absence as $key=>$val){
							...
							$this->payrollemployeemonthlyspa_model->saveNewPayrollEmployeeMonthlyAbsence($data_absence);
						}
					}
					...
					if(!empty($session_late)){
						foreach($session_late as $key=>$val){
							...
							$this->payrollemployeemonthlyspa_model->saveNewPayrollEmployeeMonthlyLate($data_late);
						}
					}

					if(!empty($session_businesstrip)){
						foreach($session_businesstrip as $key=>$val){
							$data_businesstrip = array(
								'employee_monthly_id'			=> $employee_monthly_id,
								'employee_id'					=> $data['employee_id'],
								'employee_monthly_period'		=> $data['employee_monthly_period'],
								'overtime_rate_id'				=> $val['overtime_rate_id'],
								'business_trip_destination'		=> $val['business_trip_destination'],
								'business_trip_date'			=> tgltodb($val['business_trip_date']),
								'business_trip_amount'			=> $val['business_trip_amount'],
								'data_state'					=> 0,
								'created_id'					=> $data['created_id'],
								'created_on'					=> $data['created_on']
							);
							$this->payrollemployeemonthlyspa_model->insertPayrollEmployeeMonthlyBusinessTrip($data_businesstrip);
						}
					}
					...
					$this->session->unset_userdata('Addpayrollemployeemonthlyspa');
					...
					$this->session->unset_userdata('Addpayrollemployeemonthlyspa');
					redirect('payrollemployeemonthlyspa/addPayrollEmployeeMonthly/'.$data['employee_id']);
				}else{
					$msg = "<div class='alert alert-danger'>                
								Add Data Payroll Employee Monthly UnSuccessful
							<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button></div> ";
					$this->session->set_userdata('message',$msg);
					$this->session->set_userdata('Addpayrollemployeemonthlyspa',$data);
					redirect('payrollemployeemonthlyspa/addPayrollEmployeeMonthly/'.$data['employee_id']);
				}
			}else{
				$data['password']='';
		...
		public function deletePayrollEmployeeMonthly_Data(){
			...
			if($this->payrollemployeemonthlyspa_model->deletePayrollEmployeeMonthly_Data($employee_monthly_id)){
				$auth = $this->session->userdata('auth');
				...
				$msg = "<div class='alert alert-success'>                
						...
						<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button></div> ";
				$this->session->set_userdata('message',$msg);
				redirect('payrollemployeemonthlyspa/addPayrollEmployeeMonthly/'.$employee_id);
			}else{
				$msg = "<div class='alert alert-danger'>                
		...
		public function detailPayrollEmployeeMonthly(){
			...
			$data['main_view']['payrollemployeemonthlyspa']					= $this->payrollemployeemonthlyspa_model->getPayrollEmployeeMonthly_Detail($employee_monthly_id);

			$data['main_view']['payrollemployeemonthlyspaabsence']			= $this->payrollemployeemonthlyspa_model->getPayrollEmployeeMonthlyAbsence_Detail($employee_monthly_id);

			$data['main_view']['payrollemployeemonthlyspaallowance']			= $this->payrollemployeemonthlyspa_model->getPayrollEmployeeMonthlyAllowance_Detail($employee_monthly_id);

			$data['main_view']['payrollemployeemonthlyspadayoff']			= $this->payrollemployeemonthlyspa_model->getPayrollEmployeeMonthlyDayOff_Detail($employee_monthly_id);

			$data['main_view']['payrollemployeemonthlyspadeduction']			= $this->payrollemployeemonthlyspa_model->getPayrollEmployeeMonthlyDeduction_Detail($employee_monthly_id);

			$data['main_view']['payrollemployeemonthlyspaearly']				= $this->payrollemployeemonthlyspa_model->getPayrollEmployeeMonthlyEarly_Detail($employee_monthly_id);

			$data['main_view']['payrollemployeemonthlyspahomeearly']			= $this->payrollemployeemonthlyspa_model->getPayrollEmployeeMonthlyHomeEarly_Detail($employee_monthly_id);

			$data['main_view']['payrollemployeemonthlyspalate']				= $this->payrollemployeemonthlyspa_model->getPayrollEmployeeMonthlyLate_Detail($employee_monthly_id);

			$data['main_view']['payrollemployeemonthlyspaleave']				= $this->payrollemployeemonthlyspa_model->getPayrollEmployeeMonthlyLeave_Detail($employee_monthly_id);

			$data['main_view']['payrollemployeemonthlyspaovertime']			= $this->payrollemployeemonthlyspa_model->getPayrollEmployeeMonthlyOvertime_Detail($employee_monthly_id);

			$data['main_view']['payrollemployeemonthlyspaovertimerequest']	= $this->payrollemployeemonthlyspa_model->getPayrollEmployeeMonthlyOvertimeRequest_Detail($employee_monthly_id);

			$data['main_view']['payrollemployeemonthlyspapermit']			= $this->payrollemployeemonthlyspa_model->getPayrollEmployeeMonthlyPermit_Detail($employee_monthly_id);

			$data['main_view']['content']								='payrollemployeemonthlyspa/formdetailpayrollemployeemonthlyspa_view';
			$this->load->view('mainpage_view',$data);
		} 
	}
?>