<?php
	class hroemployeedatailufa extends CI_Controller{
		...
		public function index(){
			...
			$data['main_view']['coredepartment']		= create_double($this->hroemployeedatailufa_model->getCoreDepartment(),'department_id','department_name');
			$data['main_view']['coresection']			= create_double($this->hroemployeedatailufa_model->getCoreSection(),'section_id','section_name');

			$data['main_view']['hroemployeedatailufa']	= $this->hroemployeedatailufa_model->getHROEmployeeData($region_id, $branch_id, $location_id, $payroll_employee_level, $sesi['division_id'], $sesi['department_id'] , $sesi['section_id']);
		...
		function processAddHROEmployeeData(){
				...
				if( $_POST AND $this->upload->do_upload('employee_picture') ) {
					...
					if ( ! $this->image_lib->resize()){
						...
						$data = array(
							...
							'region_id_incentive'					=> $region_id,
							'branch_id_incentive'					=> $branch_id,
							'location_id_incentive'					=> $location_id,
							'division_id_incentive'					=> $this->input->post('division_id',true),
							'department_id_incentive'				=> $this->input->post('department_id',true),
							'section_id_incentive'					=> $this->input->post('section_id',true),
							'job_title_id_incentive'				=> $this->input->post('job_title_id',true),
							'grade_id_incentive'					=> $this->input->post('grade_id',true),
							'class_id_incentive'					=> $this->input->post('class_id',true),
			...
			if($this->form_validation->run()==true){
					if($this->hroemployeedatailufa_model->saveNewHROEmployeeData($data) == true){
						$auth 	= $this->session->userdata('auth');
		...
		function processEdithroemployeedatailufa(){
			$auth = $this->session->userdata('auth');
			$region_id 		= $auth['region_id'];
			$branch_id 		= $auth['branch_id'];
			$location_id 	= $auth['location_id'];
			...
			if($this->form_validation->run()==true){
				// if($this->uploadgambar()==true){
					if($this->hroemployeedatailufa_model->saveEditHROEmployeeData($data)==true){
						$auth 	= $this->session->userdata('auth');
