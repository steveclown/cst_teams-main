	...
	<?php
		...
		if(isset($_POST)){
			...
			foreach($dataarray as $k=>$v){
				...
				if($state=="1"){
					$filename = $PNG_TEMP_DIR.'test'.md5($v['customer_code'].'|'.$errorCorrectionLevel.'|'.$matrixPointSize).'.png';
					QRcode::png($v['customer_code'], $filename, $errorCorrectionLevel, $matrixPointSize, 2);
					$datarisk = array(
						'namafile'			=>	$filename,
		...
		if(!empty($filenames)){
			foreach($filenames as $k2=>$v2){
				...
				if($_POST['printtext']=='yes'){
					echo'
							<tr>
								<td style="text-align:center;"><font size="2">'.$v2['customer_name'].'</font><BR><font size="2">'.$v2['customer_code'].'</font><BR><font size="1">'.$v2['customer_address'].'</font></td>
							</tr>
					';					
				}
				echo'
